@extends('layouts.app')
@section('title', 'Dashboard — SevaSL')
...
<div style="max-width:1280px; margin:0 auto; padding:40px 24px;">
    ...
    <div style="display:flex; align-items:center; justify-content:space-between; margin-bottom:32px; flex-wrap:wrap; gap:16px;">
        <div>
            <h1 style="font-size:32px; font-weight:700; color:var(--text); margin-bottom:4px;">
                Welcome, {{ auth()->user()->name }}!
            </h1>
            <p style="font-size:15px; color:var(--text-secondary);">Manage your services and track performance</p>
        </div>
    ...
    <div style="display:grid; grid-template-columns:repeat(4,1fr); gap:16px; margin-bottom:40px;" class="dash-stats">
        ...
        <div style="background:var(--card); border-radius:16px; border:1.5px solid var(--border); padding:20px; text-align:center;">
            <div style="font-size:32px; font-weight:700; color:var(--secondary-dark);">{{ $stats['views'] }}</div>
            <div style="font-size:13px; color:var(--text-secondary); margin-top:4px; font-weight:700;">Total Views</div>
        </div>
    </div>

    {{-- My Services --}}
    <h2 style="font-size:22px; font-weight:700; color:var(--text); margin-bottom:20px;">My Services</h2>
    ...
    <div style="background:var(--card); border-radius:16px; border:1.5px solid var(--border); padding:20px; margin-bottom:14px; display:flex; align-items:center; justify-content:space-between; gap:16px; flex-wrap:wrap; transition:all .2s;"
        ...
        <div style="text-align:center; min-width:60px;">
            <div style="font-size:11px; color:var(--text-light); font-weight:700;">VIEWS</div>
            <div style="font-weight:700; color:var(--text); font-size:16px;">{{ $service->views_count }}</div>
        </div>
    ...
    <div style="text-align:center; padding:80px; background:var(--card); border-radius:20px; border:1.5px solid var(--border);">
        ...
        <p style="color:var(--text-light); margin-bottom:20px;">Post your first service and start getting customers!</p>
        <a href="{{ route('services.create') }}"
           style="background:var(--primary); color:white; border-radius:12px; padding:12px 24px; font-size:14px; font-weight:700; text-decoration:none;">
